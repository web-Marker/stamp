import type { ICompany } from './DrawStampTypes'
...
export class DrawCompanyUtils {
  ...
  drawCompanyName(
    ...
    // 计算总角度和每个字符的角度
    const totalAngle =
      Math.PI * (0.5 + characterCount / (company.textDistributionFactor * 4))
    const anglePerChar = totalAngle / characterCount

    // 根据旋转方向设置起始角度和角度增量
    const direction = company.rotateDirection === 'clockwise' ? -1 : 1
    const startAngle =
      (company.startAngle ? company.startAngle : 0) +
      (company.rotateDirection === 'clockwise'
    ...
    if (company.adjustEllipseText) {
      ...
      characters.forEach((char, index) => {
        ...
        const additionalAngle =
          adjustmentFactor * anglePerChar * company.adjustEllipseTextFactor
        ...
        // 计算字符位置
        const x =
          centerX + Math.cos(angle) * (radiusX - fontSize - borderOffset)
        const y =
          centerY + Math.sin(angle) * (radiusY - fontSize - borderOffset)
